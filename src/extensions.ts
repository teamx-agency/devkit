 ...
 */
function parseExtensionsYaml(raw: string): ExtensionsFile {
  const lines = raw.split(/\r?\n/);
  const hooks: Record<string, ExtensionHookEntry[]> = {};

  let currentHook: string | null = null;
  let currentEntry: Partial<ExtensionHookEntry> | null = null;
  ...
  for (const rawLine of lines) {
    ...
    if (fieldMatch && currentEntry) {
      ...
      let value: string | boolean = fieldMatch[2].trim().replace(/^["']|["']$/g, '');
      if (key === 'optional') {
        value = value === 'true';
...
export function loadExtensions(cwd: string): ExtensionsFile | null {
 ...
 */
export function buildHookReport(results: ExtensionHookResult[]): string | null {
  if (results.length === 0) return null;

  const lines: string[] = [`[TeamX Extensions — ${results[0].phase}_${results[0].gate.toLowerCase()}]`];
