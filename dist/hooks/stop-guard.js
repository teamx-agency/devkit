...
export function resetBlockCount(cwd) {
...
function writeEmergencyHandoff(cwd, state) {
    ...
    lines.push(``, `## Acceptance Criteria`, t?.acceptance_criteria?.length
        ? t.acceptance_criteria.map(c => `- ${c}`).join('\n')
        : `- Not loaded — call teamx_get_task_detail(${t?.uuid ?? ''}) after resuming`, ``, `## Milestone`, `- ${state.active_milestone?.title ?? 'none'} (${state.active_milestone?.done_tasks ?? 0}/${state.active_milestone?.total_tasks ?? 0})`, ``, `## ⚠ Warning`, `Agent exited by safety valve — work at gate \`${state.current_gate}\` may be incomplete.`, `Check uncommitted changes before resuming.`);
    try {
...
export function handleStop(data) {
    ...
    if (SAFE_STOP_GATES.includes(gate)) {
        resetBlockCount(cwd);
        return { decision: 'approve' };
    }
